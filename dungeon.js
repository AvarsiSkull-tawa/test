...
function _doStartWave() {
    ...
    // --- THÊM CODE BUFF ẨM THỰC CHO DUNGEON VÀO ĐÂY ---
    if (All.getActiveCookingBuffs) {
        const buffs = All.getActiveCookingBuffs();
        if (buffs.length > 0) {
            fullTeam.forEach(p => {
                // Chỉ buff 1 lần để tránh bị cộng dồn mỗi qua ải
                if (!p._cookBuffApplied) {
                    ...
                    p.dodge = (p.dodge !== undefined ? p.dodge : (p.id === 'ghostBlob' ? 0.15 : 0.05)) + addDodge;
...
function showWaveRewards(isLoaded = false) {
    ...
    if (!isLoaded) {
        ...
        fullTeam.forEach(p => {
            ...
            // --- FIX CHỐNG MẤT BUFF TỐC ĐỘ QUA ẢI ---
            if (All.getActiveCookingBuffs) {
                let spdM = 1;
